...
This project supports **two task execution modes** in Databricks Workflows. Both delegate to shared **orchestrator modules** ([`src/bank_etl/orchestrators/`](src/bank_etl/orchestrators/)) that contain all business logic. The DAB job YAMLs include both options — comment/uncomment to switch.

| Feature | `notebook_task` (default) | `python_wheel_task` |
|---------|---------------------------|---------------------|
| Entry point | [`notebooks/`](notebooks/) | [`src/bank_etl/entry_points/`](src/bank_etl/entry_points/) |
| Business logic | Delegates to [`src/bank_etl/orchestrators/`](src/bank_etl/orchestrators/) | Delegates to [`src/bank_etl/orchestrators/`](src/bank_etl/orchestrators/) |
